...
def print_similar_vectors(similar_vectors: Dict[int, List[Tuple[str, float]]]) -> None:
    ...
    """
    for input_idx, similar_docs in similar_vectors.items():
        logging.warning(f"Results for the {input_idx+1}th input vector:")
        for doc in similar_docs:
            logging.warning(f"ID: {doc[0]}, Distance: {doc[1]}")
...
def search_similar(
    ...
    for idx, input_vector in enumerate(input_vectors):
        ...
        similar_docs = [(doc_ids[indices[0][i]], distances[0][i]) for i in range(count)]
        similar_vectors[idx] = similar_docs
